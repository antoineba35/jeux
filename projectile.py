...
class Projectile(pygame.sprite.Sprite):
    ...
    def remove(self):
        self.player.all_projectiles.remove(self)  # remove : s upprimer le projetile courant
    ...
    def move(self):
        ...
        for monster in self.player.game.check_collision(self, self.player.game.all_monsters):
            #supprimer le projectile et infliger des dégats au monstre
            self.remove()
            monster.damage(self.player.attack)

        #verifier si notre projectile n'est plus présent sur l'écran
        if self.rect.x > 1080:
            #supprimer le projectile (en dehors de notre écran)
            self.remove()